...
Upstream equivalent: none as of `origin/main` at `79b00a7` (v0.9.1), verified 2026-09-18. Keep this behavior.
...
Upstream equivalent: none as of `origin/main` at `79b00a7` (v0.9.1), verified 2026-09-18. Keep this behavior. If upstream adds clickable filenames later, prefer its implementation only when it preserves the path, line, and optional workspace contract.

## Current upstream review

Verified 2026-09-18 after fetching both remotes. `origin/main` remains at `79b00a7` (v0.9.1), so no upstream functionality arrived since the previous run. Local `main` exactly matched `fork/main`; `git log HEAD..origin/main` was empty, and rebasing onto `origin/main` was a no-op.
