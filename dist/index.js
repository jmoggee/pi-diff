...
function diffOpenUri(cwd, filePath, line, workspace) {
...
function formatToolHeaderPath(theme, filePath, cwd, line, workspace) {
    const path = theme.fg("toolTitle", filePath);
    return cwd && line ? hyperlink(path, diffOpenUri(cwd, filePath, line, workspace)) : path;
}
function diffOpenLine(diff) {
    return (diff.lines.find((line) => line.type === "add" && line.newNum !== null)?.newNum ??
...
export default async function diffRendererExtension(pi) {
    ...
    function stashEditHeaderStats(toolCallId, edits, diffLines, added, removed, line) {
        if (!toolCallId)
            return;
        editHeaderStatsByCallId.set(toolCallId, { edits, diffLines, added, removed, line });
    }
    const writeHeaderStatsByCallId = new Map();
    function stashWriteHeaderStats(toolCallId, added, removed, line) {
        if (!toolCallId)
            return;
