...
function diffOpenUri(cwd: string, filePath: string, line: number, workspace?: string): string {
	...
	registerToolIfEnabled("write", {
		...
		renderCall(args: any, theme: any, ctx: any) {
			...
			if (args?.content && !ctx.argsComplete) {
				...
				setToolHeaderBg(text);
				text.setText(
					formatToolFrameHeaderText({ label, filePath: fp, fileLine, theme, suffix, topPad: 0, bottomPad: 0 }),
				);
				return text;
			}
			...
			setToolHeaderBg(text);
			text.setText(
				formatToolFrameHeaderText({ label, filePath: fp, fileLine, theme, suffix: stats, topPad: 0, bottomPad: 0 }),
			);
			return text;
		},
